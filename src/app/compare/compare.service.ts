...
export class CompareService {
  ...
  onRetrieveData(all = true) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      ...
      this.http.get('https://5g2s6i37xd.execute-api.us-east-2.amazonaws.com/dev/compare-yourself/' + urlParam + queryParam, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
        .map(
  ...
  onDeleteData() {
    this.dataLoadFailed.next(false);
    this.authService.getAuthenticatedUser().getSession((err, session) => {
      this.http.delete('https://5g2s6i37xd.execute-api.us-east-2.amazonaws.com/dev/compare-yourself/?accessToken=XXX', {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
        .subscribe(
          (data) => {
